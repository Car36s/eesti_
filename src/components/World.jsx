...
class World extends Component {
	constructor(props) {
		...
		this.setActiveMapProperties = this.setActiveMapProperties.bind(this)
	...
	setActiveMapProperties(properties) {
	...
	setActiveTile(activeTile) {
		...
		this.svg.selectAll(selector).classed('eesti--active', function(d) {
			this.setActiveMapProperties(d.properties)
			return true
		})
	}
	...
	updateCitiesVisibility(type, visibility, properties = null) {
		...
		const setActiveMapProperties = properties => this.setActiveMapProperties(properties)
	}
	...
	componentDidMount() {
		...
		Promise.all([d3.json('../data/eesti.json'), d3.csv('../data/linnad.csv')])
			.then(([mapData, cities]) => {
				this.setState({
					mapData,
					cities
